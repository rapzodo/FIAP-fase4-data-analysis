...
class FacialRecognitionTool(BaseTool):
    ...
    def _run(self, video_path: str, sample_rate: int = 5):
        ...
        if not cap.isOpened():
            return FacialRecognitionError(error=f"Unable to open video {video_path}").model_dump_json(indent=2)

        try:
            result = self.process_face_recognition(cap, sample_rate)
        finally:
            cap.release()

        return result.model_dump_json(indent=2)

    def process_face_recognition(self, cap: cv2.VideoCapture, sample_rate: int) -> FacialRecognitionResult:
